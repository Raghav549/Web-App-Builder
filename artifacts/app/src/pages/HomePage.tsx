...
import { useGetFeed, useGetVoteStats, useCastVote } from "@workspace/api-client-react";
          ...
          <div className="bg-card border border-card-border rounded-[2rem] p-5 shadow-sm relative overflow-hidden">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <Button 
                ...
                className={cn(
                  "rounded-full h-10 px-4 shadow-sm transition-all duration-300",
                  isVoting ? "scale-95 bg-primary/80" : "hover:scale-105"
                )}
              >
                <Heart className={cn("mr-1", isVoting ? "fill-current animate-ping" : "fill-current")} size={16} />
                Cheer
              </Button>
            </div>
            ...
            {isLoadingStats ? (
              ...
              <VoteMeter stats={voteStats} showLabel={false} />
            )}
          </div>
        </div>
                ...
                <div className="cursor-pointer" onClick={() => setLocation(`/post/${post.id}`)}>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center gap-6">
                      <button className="flex items-center gap-1.5 text-muted-foreground hover:text-accent-foreground transition-colors group">
                        <div className={cn("p-1.5 rounded-full group-hover:bg-accent/20 transition-colors", post.isLiked && "text-accent-foreground")}>
